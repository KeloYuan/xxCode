...
## 2025-12-29
- **4.0.0 发布 (Release 4.0.0)**:
  - **响应式架构升级**: 新增移动端/桌面端布局切换，抽屉与底部面板适配小屏交互。
  - **多标签编辑体验**: 支持标签切换、关闭、管理面板与快速新建。
  - **代码运行与控制台**: 内置运行控制台，支持运行日志与错误输出显示。
  - **主题与视觉优化**: 深浅主题与多主题切换，界面细节与圆角风格统一。
  - **学习与模板**: 学习模式样例文件、模板库与最近项目入口完善。
